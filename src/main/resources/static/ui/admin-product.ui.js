...
// UI module cho quản lý sản phẩm admin - render HTML components
...
/**
 * Render hình ảnh sản phẩm (thumbnail)
 */
...
function normalizeImageUrl(raw) {
    ...
    if (v.startsWith('/images/') || v.startsWith('http://') || v.startsWith('https://')) {
        return v;
    }
...
/**
 * Render một row sản phẩm trong bảng
 */
...
/**
 * Render panel variants (expandable)
 */
export function renderVariantsPanel(productId, variants = [], isExpanded = false) {
    const variantRows = variants.length > 0
        ...
        : `<tr><td colspan="6" class="px-4 py-8 text-center text-slate-500">Chưa có variant nào</td></tr>`;
                    ...
                    <div class="bg-white rounded-lg border border-slate-200 overflow-hidden">
                        <table class="w-full text-sm">
                            <thead class="bg-slate-100 text-slate-600 text-xs uppercase">
                                <tr>
                                    <th class="px-4 py-2 text-left">Hình</th>
                            ...
                            <tbody class="divide-y divide-slate-100" id="variants-tbody-${productId}">
                                ${variantRows}
                            </tbody>
                        </table>
                    </div>
...
export function renderVariantRow(variant) {
    ...
    return `
        <tr class="hover:bg-slate-50" data-variant-id="${variant.variantId}">
            ...
            <td class="px-4 py-2">
                <span class="px-2 py-1 text-xs rounded ${variant.stock > 0 ? 'bg-green-100 text-green-700' : 'bg-rose-100 text-rose-700'}">
                    ${variant.stock || 0}
                </span>
            </td>
...
export function renderProductModal(product = null, categories = [], brands = []) {
    ...
    const categoryOptions = categories.map(c =>
        `<option value="${c.value}" ${product?.categoryId === c.value ? 'selected' : ''}>${c.label}</option>`
    ).join('');

    const brandOptions = brands.map(b =>
        `<option value="${b.value}" ${product?.brandId === b.value ? 'selected' : ''}>${b.label}</option>`
    ).join('');
        ...
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" id="product-modal">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-hidden animate-fade-in">
                ...
                <form id="product-form" class="p-6 space-y-4 overflow-y-auto max-h-[calc(90vh-140px)]">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3"
                            class="w-full px-4 py-2 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">${product?.description || ''}</textarea>
                    </div>
                ...
                <div class="px-6 py-4 border-t border-slate-200 flex justify-end gap-3">
                    ...
                    <button type="submit" form="product-form" class="px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition font-medium">
            ...
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-md animate-fade-in">
                ...
                <form id="variant-form" class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="stock" value="${variant?.stock || 0}" min="0"
                            class="w-full px-4 py-2 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">URL hình ảnh</label>
                        <input type="text" name="imageUrl" value="${variant?.imageUrl || ''}"
                            class="w-full px-4 py-2 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                ...
                <div class="px-6 py-4 border-t border-slate-200 flex justify-end gap-3">
                    <button type="button" class="close-modal-btn px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-xl transition">Hủy</button>
                    <button type="submit" form="variant-form" class="px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition font-medium">
                        ${isEdit ? 'Lưu thay đổi' : 'Thêm variant'}
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" id="delete-modal">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm animate-fade-in">
                <div class="p-6 text-center">
                    ...
                    <p class="text-slate-600 mb-6">Bạn có chắc muốn xóa ${typeText} <strong>${name}</strong>? Hành động này không thể hoàn tác.</p>
